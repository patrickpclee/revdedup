...
static void segmentExist(const char * name, uint32_t seg_cnt, Segment * segs) {
	...
	strcpy(req->headers[0].value, name);
...
static void * putSegment(void * ptr) {
	...
	while (1) {
		...
		strcpy(req->func, "segmentfp");
		req->header_cnt = 1;
		strcpy(req->headers[0].key, "SegmentFP");
		strcpy(req->headers[0].value, seg->segmentfp);
		req->request_size = seg->blocks * FP_SIZE;
		req->request = seg->blockfp;
		hrs->postData(req);
