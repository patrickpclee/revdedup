...
static void getAllImages() {
	...
	mongo_cursor_init(cursor, &service.resource[0].conn, "rdserver.image");
...
static void putAllImages() {
	...
	mongo_cmd_drop_collection(&resource->conn, "rdserver", "imgver", result);
	mongo_cmd_drop_collection(&resource->conn, "rdserver", "image", result);
	...
	mongo_insert_batch(&resource->conn, "rdserver.image", (const bson **)img_bsons, img_ptr,
			NULL, 0);
...
static void getAllImageVersions() {
	...
	mongo_cursor cursor[1];
	mongo_cursor_init(cursor, &service.resource[0].conn, "rdserver.imgver");
...
static void putImageVersions(DBResource * resource, Image * img) {
	...
	mongo_insert_batch(&resource->conn, "rdserver.imgver", iv_bsons, iv_ptr,
			NULL, 0);
}
/*
...
static void getAllSegments() {
	mongo_cursor cursor[1];
	mongo_cursor_init(cursor, &service.resource[0].conn, "rdserver.segment");
...
static void putAllSegments() {
	...
	mongo_cmd_drop_collection(&resource->conn, "rdserver", "segment", result);
	uint64_t seg_ptr = 0;
	bson ** seg_bsons = malloc(seg_cnt * sizeof(bson *));
	for (i = 0; i < SEGMENT_HASH_TABLE_SIZE; i++) {
		hlist_for_each_entry(seg, node, &segment_ht[i].head, node)
		{
			...
			seg_bsons[seg_ptr++] = seg_bson;
		}
	}
	mongo_insert_batch(&resource->conn, "rdserver.segment", (const bson **)seg_bsons, seg_cnt,
			NULL, 0);
	for (i = 0; i < seg_cnt; i++) {
		free(seg_bsons[i]);
	}
